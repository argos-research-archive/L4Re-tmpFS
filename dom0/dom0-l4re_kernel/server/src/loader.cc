                               ...
                               unsigned flags, char const *what)
{
  ...
  chksys(_rm->attach(&addr, size, flags, ds, offset), what);
}
...
L4Re_app_model::copy_ds(Dataspace dst, unsigned long dst_offs,
                        ...
                        unsigned long size)
{
  if (dst->copy_in(dst_offs, src, src_offs, size) < 0)
    {
                                ...
                                unsigned long offset) const
{
  ...
  chksys(_rm->attach(&vaddr, pg_size,
	...
	ds, pg_offset), "ELF loader: attach temporary VMA");
...
L4Re_app_model::alloc_app_stack()
{
  ...
  chksys(_rm->attach(&_s, _stack.stack_size(), Rm::Search_addr, stack, 0));
...
bool Loader::start(Cap<Dataspace> bin, Region_map *rm, l4re_aux_t * /*aux*/)
{
  ...
  __loader_stack_p
	...
	Loader_stack_size, Region_handler(__loader_stack, __loader_stack.cap()),
	0, Region_map::Search);
  ...
  L4Re::Env::env()->rm()->attach(&__loader_stack_p, Loader_stack_size, 0, __loader_stack, 0);
...
#if 0
  L4::cout << "l4re: start file " << bin << " entry=" << (void*)entry.entry << '\n';
#endif
