...
int main(int argc, char const *argv[])
{
  ...
  //Mounting does not work in l4re_kernel, no matter which FS
  //->Starting of network-received binaries stored in
  //(machine-global) tmpfs (here referenced as l4fs)
  //not (yet) possible
  //mount("none","/net-files","l4fs",0,0);
  //perror("mount");
  //Even mounting the "normal" tmpfs doesn't work
  //mount("none","tmp","tmpfs",0,0);
  //perror("mount");
  //Manually mounting was tried for easier debugging
  //If it would work, one could use automatic mounting
  //by linking l4re_kernel with libmount and
  //specifying a fstab file.

//Code for the old shared-ds approach
#if 0
